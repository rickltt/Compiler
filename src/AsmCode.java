...
public class AsmCode {
    ...
    public void cToAsm(Syntax Cgramma,SymbolTable table){
        ...
        for(int i=0;i<table.Synbl.size();i++){
            if(table.Synbl.get(i).type.equals("struct")){//如果是struct的情况
                ...
                for(int j=0;j<table.Rinfl.size();j++){
                    if(table.Rinfl.get(j).type.equals("int")||table.Rinfl.get(j).type.equals("char")||table.Rinfl.get(j).type.equals("string")){
        ...
        for(int i=0;i<Cgramma.qt.size();i++){
            ...
            else if(Cgramma.qt.get(i).First.equals("SCANF")){
                        asmCode.add("      MOV "+RDL+ ",AL");
                        asmCode.add("      XOR AH,AH");
                        asmCode.add("      MOV AH,01H");//DOS1号，获取键盘的输入(存入AL中)
                        asmCode.add("      int 21H");
                        asmCode.add("      SUB AL,30H");
                        asmCode.add("      MOV " + Cgramma.qt.get(i).Fourth + ",AL");
            ...
            else if(Cgramma.qt.get(i).First.equals("PRINTF")){
                asmCode.add("      MOV BL,"+ Cgramma.qt.get(i).Fourth);
                asmCode.add("      ADD BL,30H");
                asmCode.add("      MOV AH,02H");//DOS2号功能，进行输出显示
                asmCode.add("      MOV DL,BL");
                asmCode.add("      int 21H");
                asmJump[i] = asmCount;
                asmCount += 5;
